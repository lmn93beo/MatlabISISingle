function [sdom, tdom, x_ecc, y_ecc] = makeGraterDomain_beta(xN,yN,ori,s_freq,t_period,altazimuth,x_size,y_size)
...
switch altazimuth
    ...
    case 'altitude'
        ...
        %%%Get the xy domain
        x_ecc = single(tan(x_size/2*pi/180)*Mstate.screenDist);  %cm
        y_ecc = single(tan(y_size/2*pi/180)*Mstate.screenDist);
