function DotFrame = makeBufferRandomDots(x_size,y_size,ori,density,speed,dotlifetime,coherence)
...
[screenXmm, ~] = Screen('DisplaySize',screenNum); 

%this calculation is based on the assumption that the screen is round
pxDeg = 2*pi/360*Mstate.screenDist*screenRes.width/screenXmm * 10;  % pixels per degree
...
nrDots=round(density*stimArea/fps); %this is the number of dots in each frame
...
deltaFrame = speed*pxDeg/fps;                            % dot speed (pixels/frame)
deltaX=deltaFrame*cos(ori*pi/180);
deltaY=-1*deltaFrame*sin(ori*pi/180);

%initialize lifetime vector - between 1 and dotLifetimte
if dotlifetime>0
    randlife=randi(s,dotlifetime,nrDots,1);
    lifetime=randlife;
end

%initialize signal/noise vector; 1 indicates signal, 0 indicates noise
nrSignal=round(nrDots*coherence/100);
...
for i=1:nrFrames
    ...
    xproj=cos(ori*pi/180);
    yproj=-sin(ori*pi/180);
